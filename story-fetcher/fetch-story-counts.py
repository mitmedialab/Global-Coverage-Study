...
@cache
def get_story_from_topic(stories_id):
    return mc.topicStoryList(TOPIC_ID, stories_id=stories_id)['stories'][0]

@cache
def get_stories(query, process_stories_id):
    return mc.storyList(solr_query=query,
        last_processed_stories_id=process_stories_id, rows=STORIES_PER_PAGE,
        ap_stories_id=1)
        ...
        try:
            ...
            if len(stories) > 0:
                for idx, story in enumerate(stories):
                    topic_story = get_story_from_topic(story['stories_id'])
