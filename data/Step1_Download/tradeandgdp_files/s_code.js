...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,swf,png,csv"
s.linkInternalFilters="javascript:,imf.org,www.imf.org,www.internationalmonetaryfund.com,www.internationalmonetaryfund.org"
...
s.visitorNamespace="imf"
s.dc=112

/* Uncomment these lines to use first party cookies after the SSL is installed and cnames set up */
s.trackingServer="webstats.imf.org"
s.trackingServerSecure="swebstats.imf.org"
...
function s_doPlugins(s) {
	...
	if(!s.pageType && !s.pageName){
		s.FullPageName=s.getPageName().toLowerCase() + window.location.search.toLowerCase() + document.title;
		s.pageName=s.FullPageName.substring(0,100)
		s.prop8=window.location.search;
	...
	/* Set search term based on "NewQuery" parameter */
	s.prop4=s.getQueryParam('NewQuery');
	...
	if(s.prop4){
		...
		s.prop6=s.getQueryParam('lan')
		s.prop7=s.getQueryParam('col')
		}
	
	/* Set event 1 on every page view */
	s.events=s.events?s.events+',event1':'event1';
	
	//determine language code
	s.ref=location.href;
	s.ref.toLowerCase();
	if(s.ref.indexOf("/fra")>-1||s.ref.indexOf("/fre")>-1||s.ref.indexOf("/french")>-1){s.prop5="French"}
	else if(s.ref.indexOf("/esl")>-1||s.ref.indexOf("/spa")>-1||s.ref.indexOf("/spanish")>-1){s.prop5="Spanish"}
	else if(s.ref.indexOf("/chi")>-1||s.ref.indexOf("/chn")>-1||s.ref.indexOf("/chinese")>-1){s.prop5="Chinese"}
	else if(s.ref.indexOf("/jpn")>-1||s.ref.indexOf("/japanese")>-1){s.prop5="Japanese"}
	else if(s.ref.indexOf("/rus")>-1||s.ref.indexOf("/russian")>-1){s.prop5="Russian"}
	else if(s.ref.indexOf("/ara")>-1||s.ref.indexOf("/arabic")>-1){s.prop5="Arabic"}
	else {s.prop5="English"}
